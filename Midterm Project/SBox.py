from Permutation import rockin_dat_perm
#from KeySchedule import 
from  PRNG import TRNG
from KeySchedule import key_expansion
import numpy as math #key expansion
...
reverse_sbox = [
    ...
    #this method aims to make it nonlinear
def foward_sub(data): #prepares value for sbox
    return forward_sbox[data>>4][data&15] #two dimensional array for sbox
    #acts like 4bits in, 4bits out, right shifted for x value
    #2^4 possible values, AND for y value


def reverse_sub(data):
    return reverse_sbox[data>>4][data&15] #two dimensional array for sbox
    #acts like 4bits in, 4bits out
    #2^4 possible values

for round in range(2): #for two rounds
    def encoding(secret_msg, key):
        converted_hex = [] #start of unicode hex int cast conversion
        encoded_values = [] #array that stores encoded XOR values
        sbox_msg = [] #array that stores sbox values of encoded elements
        
        for index, l in enumerate(secret_msg):
            converted_hex.append(int(l.encode('utf-8').hex(), 16)) #isn't "needed", just easier to track changes
            encoded_values.append((index%key^round)^converted_hex[index]) #encoded value added to array, XORed with char index to prevent collisions
                #key expansion depending on round
            #encodes first character in unicode, converted to hex, cast as an int
            sbox_msg.append(foward_sub(encoded_values[index])) #encoded value put through first stage of sbox

        print("Converted hex: ", converted_hex)
        print("Encoded, XORed: ", encoded_values)
        print("Sbox output: ", sbox_msg)

        decoding(rockin_dat_perm(sbox_msg, key), key) #method to call if want to decode the sbox
     
     
     
     
        
def hex_back_to_message(hex):
        if hex[:2] == '0x': #parses 0x of hex if exists
            hex = hex[2:] #discards 0x at beginning
        return bytes.fromhex(hex).decode('utf-8') #converts from utf-8 back to characters

  
def decoding(unpermuted_array, key): 
    for round in range(2): #for two rounds
        reverse_msg = [] #stores value reversion of encoded_values[]
        reverse_xor = [] #reverts value back to converted_hex[]
        
        for index, r in enumerate(unpermuted_array):
            reverse_msg.append(reverse_sub(r)) #reverts value back to encoded_values[]
            reverse_xor.append((index%key^round)^reverse_sub(r)) #reverts value back to converted_hex[]
            
        
    print("Reverse sbox: ", reverse_msg)
    print("Reverse XOR to original starting: ", reverse_xor)

    print("\nFull decoded message is: ")
    for index, num in enumerate(reverse_xor): #iterates through last array to decode unicode characters
        #final line of code that is run on this page, prints decrypted message to screen
        print(hex_back_to_message(hex(num)), end= '') #utf-8 values represented as hex, calls method to convert back
    print("\n------------------------------------------------------------------") 
